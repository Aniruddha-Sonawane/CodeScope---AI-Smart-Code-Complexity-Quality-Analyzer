...
const COLORS = {
  ...
  primary: "#6366f1", accent: "#22d3ee", danger: "#ef4444",
  modalOverlay: "rgba(0, 0, 0, 0.9)"
};

// ─── All languages supported by lizard ──────────────────────────────────────
// monacoId: the language string Monaco editor understands
// ext:      file extension lizard uses to detect the language
const LANGUAGES = [
  ...
  { value: "c",           label: "C",             monacoId: "c",           ext: ".c"    },
  { value: "cpp",         label: "C++",           monacoId: "cpp",         ext: ".cpp"  },
  { value: "csharp",      label: "C#",            monacoId: "csharp",      ext: ".cs"   },
  { value: "go",          label: "Go",            monacoId: "go",          ext: ".go"   },
  { value: "rust",        label: "Rust",          monacoId: "rust",        ext: ".rs"   },
  { value: "swift",       label: "Swift",         monacoId: "swift",       ext: ".swift"},
  { value: "ruby",        label: "Ruby",          monacoId: "ruby",        ext: ".rb"   },
  { value: "php",         label: "PHP",           monacoId: "php",         ext: ".php"  },
  { value: "scala",       label: "Scala",         monacoId: "scala",       ext: ".scala"},
  { value: "kotlin",      label: "Kotlin",        monacoId: "kotlin",      ext: ".kt"   },
  { value: "objectivec",  label: "Objective-C",   monacoId: "objective-c", ext: ".m"    },
  { value: "lua",         label: "Lua",           monacoId: "lua",         ext: ".lua"  },
  { value: "plsql",       label: "PL/SQL",        monacoId: "sql",         ext: ".sql"  },
  { value: "GDScript",    label: "GDScript",      monacoId: "gdscript",    ext: ".gd"   },
];
...
const DEFAULT_SNIPPETS: Record<string, string> = {
  ...
  plsql:      `CREATE OR REPLACE PROCEDURE example IS\nBEGIN\n    FOR i IN 0..9 LOOP\n        IF MOD(i, 2) = 0 THEN\n            DBMS_OUTPUT.PUT_LINE(i);\n        END IF;\n    END LOOP;\nEND;`,
  GDScript:   `func example():\n    for i in range(10):\n        if i % 2 == 0:\n            print(i)`,
};
// ────────────────────────────────────────────────────────────────────────────
...
const modalStyle: React.CSSProperties = {
  ...
  borderRadius: "20px", padding: "24px", zIndex: 1001,
  width: "90%", maxWidth: "850px", height: "500px",
  display: "flex", flexDirection: "column",
  boxShadow: "0 25px 50px -12px rgba(0,0,0,0.8)", overflow: "hidden"
};
const overlayStyle: React.CSSProperties = {
  position: "fixed", top: 0, left: 0, right: 0, bottom: 0,
  background: COLORS.modalOverlay, backdropFilter: "blur(8px)", zIndex: 1000
};
...
function badge(text: string) {
  ...
  return (
    <span style={{ background: `${color}15`, color, padding: "4px 10px",
      borderRadius: "12px", fontSize: "0.7rem", fontWeight: "bold",
      border: `1px solid ${color}33` }}>{text}</span>
  );
}

function riskColor(riskText: string) {
  const r = riskText.toLowerCase();
  if (r.includes("high"))   return COLORS.danger;
...
export default function App() {
  ...
  const handleLanguageChange = (lang: string) => {
    setLanguage(lang);
    setCode(DEFAULT_SNIPPETS[lang] || "// Start coding...");
    setResult(null);
  };
  ...
  function downloadReport() {
    ...
    doc.setFontSize(9); doc.setTextColor(148, 163, 184); doc.text("QUALITY GRADE", 20, yPos);
    ...
    doc.setFontSize(9); doc.setTextColor(148, 163, 184); doc.text("RISK LEVEL", 20 + colW, yPos);
    doc.setFontSize(12);
    const isHigh = result.risk.toLowerCase().includes("high");
    doc.setTextColor(isHigh ? 220 : 16, isHigh ? 38 : 185, isHigh ? 38 : 129);
    doc.text(cleanStr(result.risk), 20 + colW, yPos + 7);

    doc.setFontSize(9); doc.setTextColor(148, 163, 184); doc.text("COMPLEXITY", 20 + colW * 2, yPos);
    doc.setFontSize(12); doc.setTextColor(34, 211, 238);
    doc.text(cleanStr(result.time_complexity), 20 + colW * 2, yPos + 7);

    yPos += 25;
    doc.setDrawColor(226, 232, 240); doc.line(20, yPos, pageWidth - 20, yPos);
    yPos += 15;
    ...
    result.explanations.forEach((obs: string) => {
      const split = doc.splitTextToSize(`> ${cleanStr(obs)}`, pageWidth - 40);
    ...
    autoTable(doc, {
      ...
      body: result.functions.map((f: any) => [
        cleanStr(f.name), f.line, f.complexity,
  ...
  const ChartContent = ({ height = 200, data = [] }: { height?: number; data?: any[] }) => (
    <ResponsiveContainer width="100%" height={height}>
      <BarChart data={data}>
        ...
        <Bar dataKey="complexity" radius={[6,6,0,0]} barSize={40}
          onClick={(d: any) => { if (d?.payload?.line) handleFunctionClick(d.payload.line); }}
          style={{ cursor: "pointer" }}>
          {data.map((entry: any, i: number) => (
            <Cell key={i} fill={entry.complexity > 5 ? COLORS.danger : COLORS.primary} />
          ))}
        </Bar>
      </BarChart>
    </ResponsiveContainer>
  ...
  const StatsCards = () => (
    <>
      <div className="card">
        <p style={{ color: COLORS.textMuted, fontSize: "1rem", fontWeight: 700, marginBottom: "14px", letterSpacing: "0.05em" }}>RISK ASSESSMENT</p>
        {badge(result.risk)}
      </div>
      <div className="card">
        <p style={{ color: COLORS.textMuted, fontSize: "1rem", fontWeight: 700, marginBottom: "14px", letterSpacing: "0.05em" }}>CONFIDENCE</p>
        <b style={{ fontSize: "1.6rem", fontWeight: 800,
          color: result.confidence >= 80 ? "#10b981" : result.confidence >= 60 ? "#facc15" : COLORS.danger }}>
          {result.confidence}%
        </b>
      </div>
      <div className="card">
        <p style={{ color: COLORS.textMuted, fontSize: "1rem", fontWeight: 700, marginBottom: "14px", letterSpacing: "0.05em" }}>OVERALL GRADE</p>
        {badge(result.quality_grade)}
      </div>
      <div className="card">
        <p style={{ color: COLORS.textMuted, fontSize: "1rem", fontWeight: 700, marginBottom: "14px", letterSpacing: "0.05em" }}>COMPLEXITY</p>
        <b style={{ color: COLORS.accent, fontSize: "1.1rem" }}>{result.time_complexity}</b>
      </div>
    </>
  );
    ...
    <div className="app">
      ...
      <nav className="topbar">
        <div onClick={() => setIsSearchVisible(!isSearchVisible)} className="brand">
          ...
          <span className="brand-title">AI Based Code Complexity and Risk Prediction System Using Machine Learning</span>
        </div>
        <div className="topbar-center" />
        <div className="topbar-actions">
          <span className="topbar-byline">By <b style={{ color: COLORS.textMain }}>Aniruddha Sonawane</b></span>
          <a href="https://github.com/Aniruddha-Sonawane" target="_blank" className="pill-link">
            <img src={githubLogo} alt="GitHub" style={{ width: 18, height: 18 }} /> GitHub
          </a>
          <a href="https://www.linkedin.com/in/AniruddhaSonawane1" target="_blank" className="pill-link">
            <img src={linkedinLogo} alt="LinkedIn" style={{ width: 18, height: 18 }} /> LinkedIn
          </a>
        </div>
      </nav>
      ...
      {isSearchVisible && (
        <>
          <div style={{ position: "fixed", inset: 0, background: "transparent", zIndex: 1001 }}
               onClick={() => setIsSearchVisible(false)} />
          <div style={{ position: "fixed", top: "10px", left: "50%",
            transform: "translateX(-18.75%) translateY(12.5%)", width: "min(920px, 92vw)", zIndex: 1002 }}>
            <input autoFocus type="text" placeholder="Filter functions..."
            ...
            <div className={`editor-and-stats ${isEditorExpanded ? "expanded" : "collapsed"}`}>
              <div className="card editor-card">
                  ...
                  {LANGUAGES.map(lang => (
                    <button key={lang.value} onClick={() => handleLanguageChange(lang.value)}
                      style={{
                        ...
                        fontSize: "0.75rem", fontWeight: language === lang.value ? 700 : 400,
                ...
                <div className="editor-shell">
                  <Editor height="420px" language={monacoLanguage} value={code}
                    theme="vs-dark" onChange={v => setCode(v || "")}
                ...
                <div className="actions">
                  ...
                  <button onClick={() => setIsEditorExpanded(!isEditorExpanded)} className="btn btn-ghost"
                    title={isEditorExpanded ? "Minimize" : "Expand"}>
              ...
              {!isEditorExpanded && result && (
                <div className="stats-column"><StatsCards /></div>
              )}
            </div>
            {isEditorExpanded && result && (
              <div className="stats-grid"><StatsCards /></div>
            )}
          </div>
          ...
          <div style={modalStyle}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "25px" }}>
              ...
              <button onClick={() => setIsHistoryOpen(false)}
                style={{ background: "transparent", border: "none", color: COLORS.textMuted, fontSize: "2rem", cursor: "pointer" }}>
                &times;
              </button>
            </div>
            <div style={{ overflowY: "auto", flex: 1, minHeight: 0, paddingRight: "8px" }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px" }}>
                {history.map(item => (
                  <div key={item.id}
                    onClick={() => { setCode(item.code); setResult(item.result); if (item.language) setLanguage(item.language); setIsHistoryOpen(false); }}
                    ...
                    {item.language && (
                      <span style={{ marginLeft: "8px", fontSize: "0.7rem", color: COLORS.accent,
                        background: `${COLORS.accent}15`, padding: "2px 8px", borderRadius: "10px",
        ...
        <>
          ...
          <div style={modalStyle}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "20px" }}>
              <h3 style={{ margin: 0, fontSize: "1.1rem", color: COLORS.textMain }}>Complexity Analysis</h3>
              <button onClick={() => setIsGraphZoomOpen(false)}
                ...
                <div style={{ overflowY: "auto", flex: 1, paddingRight: "8px" }}>
                  <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                    {filteredFunctions.map((fn: any, idx: number) => (
                      <div key={idx} onClick={() => handleFunctionClick(fn.line)}
                        style={{ padding: "10px 14px", background: "rgba(255,255,255,0.02)", borderRadius: "8px",
                          border: `1px solid ${COLORS.border}`, display: "flex", justifyContent: "space-between",
                          alignItems: "center", cursor: "pointer" }}
                        onMouseEnter={e => (e.currentTarget.style.background = "rgba(255,255,255,0.05)")}
                        onMouseLeave={e => (e.currentTarget.style.background = "rgba(255,255,255,0.02)")}>
                        <span style={{ fontSize: "0.85rem", fontWeight: "500", color: COLORS.textMain,
